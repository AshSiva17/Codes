...
def invert(raw: List[List[List[int]]])->None:
     ...
     for pixels in raw:
          for pixel in pixels:
               for i in range(3):
                    max_val = max(pixel[i],max_val)
                    min_val = min(pixel[i],min_val)
               for i in range(3):
                    if pixel[i] == max_val:
                         pixel[i] = min_val
...
def merge(raw1: List[List[List[int]]], raw2: List[List[List[int]]])-> List[List[List[int]]]:
    ...
    if raw1 in ([],[[]], [[[]]]) and raw2 not in ([],[[]], [[[]]]):
        new_raw = raw2.copy()
        return new_raw
    elif raw2 in ([],[[]], [[[]]]) and raw1 not in ([],[[]], [[[]]]):
        new_raw = raw1.copy()
        return new_raw
    elif raw1 in ([],[[]], [[[]]]) and raw2 in ([],[[]], [[[]]]):
        new_raw = []
        return new_raw
    
    a = max(len(raw1),len(raw2))
    b = max(len(raw1[0]),len(raw2[0]))

    
    for i in range(a):
        new_rawsub = []
        for j in range(b):
            new_rawsub.append([0,0,0])
        new_raw.append(new_rawsub)
            

    for i in range(a):
        new_rawsub = []
        for j in range(b):
            if i < min(len(raw1), len(raw2)) and j < min (len(raw1[0]),len(raw2[0])):
                if i%2 == 0:
                    new_raw[i][j] = raw1[i][j]
                else:
                    new_raw[i][j] = raw2[i][j]
...
def compress(raw: List[List[List[int]]])-> List[List[List[int]]]:
    ...
    if raw in ([],[[]], [[[]]]):
        return []
    new_first = []
    for i in range(0,len(raw),-(-len(raw)//2)):
        new_second=[]
        for j in range(0,len(raw[0]),-(-len(raw[0])//2)):
            new_third=[]
            count = 1
            for k in range(3):
                total = raw[i][j][k]
                ...
                if (j+1) < len(raw):
                    total += raw[i][j+1][k]
                    count += 1
                if (j+1) < len(raw[0]) and (i+1) < len(raw):
                    total += raw[i+1][j+1][k]
                    count += 1
                avg = total//count
                count = 1
                new_third.append(avg)
            new_second.append(new_third)
        new_first.append(new_second)
